...
export const telemetryReports = pgTable(
  ...
  (table) => ({
    telemetryDeviceTimeIdx: index('telemetry_reports_device_time_idx').on(
      table.deviceId,
      table.timestamp,
    ),
  }),
...
export const guardianE2eDeviceState = pgTable(
  'guardian_e2e_device_state',
  {
    deviceId: varchar('device_id', { length: 64 })
      .primaryKey()
      .references(() => devices.id, { onDelete: 'cascade' }),
    maxSeqAccepted: bigint('max_seq_accepted', { mode: 'bigint' })
      .notNull()
      .default(0n),
    lastValidReceivedAt: timestamp('last_valid_received_at', {
      withTimezone: true,
    }),
    metadata: jsonb('metadata')
      .$type<Record<string, unknown>>()
  ...
  (table) => ({
    guardianE2eMaxSeqIdx: index('guardian_e2e_device_state_max_seq_idx').on(
      table.maxSeqAccepted,
    ),
  }),
);
...
export const trackHistory = pgTable(
  ...
  (table) => ({
    trackHistoryAssetTimeIdx: index('track_history_asset_time_idx').on(
      table.assetId,
      table.timestamp,
    ),
  }),
...
export const trackEvents = pgTable('track_events', {
  id: varchar('id', { length: 64 }).primaryKey(),
  trackSegmentId: varchar('track_segment_id', { length: 64 }).references(
    () => trackSegments.id,
  ),
...
export const commands = pgTable(
  'commands',
  {
    ...
    issuedByUserId: varchar('issued_by_user_id', { length: 64 }).references(
      () => users.id,
    ),
...
export const missions = pgTable('missions', {
  ...
  assignedUnits: jsonb('assigned_units')
    .$type<string[]>()
    .notNull()
    .default(sql`'[]'::jsonb`),
  metadata: jsonb('metadata')
